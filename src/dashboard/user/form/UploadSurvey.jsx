...
import { Form, Upload, Input, message, Button, Typography, Space } from "antd";
...
const UploadSurvey = ({ form, onAudioChange }) => {
    ...
    try {
      ...
      form.setFieldValue(fieldName, [
        ...currentValue.filter((f) => f.uid !== file.uid && f.name !== file.name),
        {
          ...
          status: "done",
          percent: 100,
          originFileObj: actualFile instanceof File ? actualFile : fileObj, // Always preserve File object
        },
      ]);
  ...
  const handleRemove = async (file, fieldName) => {
    ...
    if (fileUrl || key) {
      ...
      try {
        // Backend delete expects { fileUrl }; some backends also accept { key }
